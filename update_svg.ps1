$file = "c:\Users\user\Desktop\sambufer\app.js"
...
$regex = [regex]::Escape("function makeSvg(name, category, id) {") + "(?s).*?" + [regex]::Escape("return 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(svg)));" + "`r`n}")
$content = [regex]::Replace($content, $regex, $newMakeSvg)
